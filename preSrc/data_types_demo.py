...
annotated_optional: str | None = None
annotated_union: int | str = "可以是 int 或 str"
...
def print_type(name: str, value) -> None:
    """打印变量的值、类型和 ID"""
    print(f"{name:25s} = {value!s:40s} -> 类型: {type(value).__name__:15s} id: {id(value)}")
...
# ---------- 4. 其他类型 -> 字符串 str ----------
print("\n--- 4. 转换为 str ---")
print(f"int(42)           -> str: {42!s}")
print(f"float(3.14)       -> str: {3.14!s}")
print(f"bool(True)        -> str: {True!s}")
print(f"list([1,2,3])     -> str: {[1, 2, 3]!s}")
print(f"tuple((1,2))      -> str: {(1, 2)!s}")
print(f"dict({{'a':1}})   -> str: { {'a': 1}!s}")
print(f"bytes(b'hello')   -> str: {b'hello'!s}")  # 默认 utf-8
chinese_bytes = "你好".encode()
...
# ---------- 11. 其他类型 -> 字节 bytes ----------
print("\n--- 11. 转换为 bytes ---")
print(f"str -> bytes (encode): {'你好'.encode()}")
print(f"list of ints -> bytes: {bytes([72, 101, 108, 108, 111])}")
print(f"bytes -> bytes: {b'hello'}")
